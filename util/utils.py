...
def init_logger(name_of_model = ""):
    ...
    with open(join(path, 'log_config.json')) as json_file:
        ...
        #filename = ''.join([filename, "_", time.strftime("%d_%m_%y_%H%M", time.localtime()), ".log"])
        filename = ''.join([time.strftime("%y%m%d_%H%M", time.localtime()), ".log"])
    
        # Creating logs' folder is needed
        log_path = create_output_dir(''.join(['/workspace/HAR_code/out/',name_of_model,'_',filename[:-4], '/']))

        log_config_dict.get('handlers').get('file_handler')['filename'] = join(log_path, ''.join([name_of_model,'_',filename]))
